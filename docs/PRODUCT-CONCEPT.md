...
**Spark Prompts (first validation mode — working name)** is Meetra’s **state-driven behavioral system** for the room: prompts are **behavioral triggers** (not passive content) — typed (icebreaker, directional, deepening, named soft-anchor, momentum), **timed to event/user state**, with **feedback** (e.g. did it / skip) shaping what comes next. It runs on **social handle** plus **lightweight tonight context**; **interaction-first**, not browsing. It isolates *does Meetra improve in-room conversation?* without **QR** or heavier game engines as validation prerequisites. **Mystery Match** is **not** replaced: Spark **validates the foundation**; Mystery **adds flagship** value **on top**. Full spec: [skills/spark-prompts.md](skills/spark-prompts.md).
