...
**Product vision:** Meetra will move toward a **layered, phase-based profile** (Lite → Social → Full). During the event the UI should behave as a **conversation engine** (talk hooks, vibe, icebreakers), not a static résumé. See [progressive-profile.md](./progressive-profile.md). This sits on top of the two-level onboarding model ([onboarding-model.md](./onboarding-model.md)) and supports mode behavior such as [Mystery Match](./mystery-match.md). Today the app uses a **single static profile** for all contexts.
